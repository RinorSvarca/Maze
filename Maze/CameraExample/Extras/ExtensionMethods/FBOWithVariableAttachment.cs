namespace GraphicsWar.ExtensionMethods
{
    ...
    public class FBOWithVariableAttachment : Disposable, IRenderSurface
    {
        /// <summary>
        ...
        /// </exception>
        public FBOWithVariableAttachment(ITexture2D texture)
        {
        ...
        /// <summary>
        /// Attaches the specified texture. The FBO will try to dispose the texture when the FBO is disposed. This texture will be added as the next colorAttachment
        /// </summary>
        ...
        {
            Attach(texture, FramebufferAttachment.ColorAttachment0 + drawBuffers.Length);
        }
        ...
        /// Given texture is null or texture dimensions do not match primary texture
        /// </exception>
        public void Attach(ITexture2D texture, FramebufferAttachment framebufferAttachment)
        {
            ...
            if(framebufferAttachment >= FramebufferAttachment.ColorAttachment0 && framebufferAttachment <= FramebufferAttachment.ColorAttachment15)
            {
                attachmentIsColor.Add(true);
            }
            else
            {
                attachmentIsColor.Add(false);
            }

            Draw(() =>
            {
                GL.FramebufferTexture2D(FramebufferTarget.Framebuffer, framebufferAttachment, TextureTarget.Texture2D, texture.ID, 0);
            ...
            for (int i = 0, k = 0; i < attachments.Count; ++i)
            {
                if(attachmentIsColor[i])
                {
                    drawBuffers.Add(DrawBuffersEnum.ColorAttachment0 + k);
                    k++;
                }
            }
        ...
        private List<bool> attachmentIsColor = new List<bool>();
